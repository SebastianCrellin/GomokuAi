...
# Tested against basic A* Search and repeatedly beat it
...
class Player(GomokuAgent):
    ...
    # this recursivly crates an n-ary tree with all possible board positions then uses a minimax algorithem to pick a move
    def depth(self, start, depth, color, alpha, beta):
        ...
        # for the color of the player crates children nodes of posibul bourd positions and adds them to the strat node in the n-ary tree
        if color == self.ID:
            # sets the max value to -inf
            max_val = float('-inf')

            # calls bourdStates and saves the return in an array
            ...
            for i in range(len(storedGames)):
                ...
                # crates a child node that stores a posibul bourd position and the corresponding move
                child1 = NaryTreeNode(storedGames[i][0], storedGames[i][1])

                # recursivly calls depth until depth = 0
                value = self.depth(child1, depth - 1, -color, alpha, beta)

                # the max part of the minimax algorithem used to determin the move chosen
                if value > max_val:
                    max_val = value
                    if depth == 2:
                ...
                # adds the cild to the n-ary tree
                start.add_child(child1)
            return max_val

        # for the color of the oponant crates children nodes of posibul bourd positions and adds them to the strat node in the n-ary tree
        else:
            ...
            for i in range(len(storedGames)):
                ...
                # the min part of the minimax algorithem used to determin the move chosen
                if value < min_val:
                    min_val = value
                    if depth == 2:
                        savedGrid = child1.get_move()
...
class NaryTreeNode:
    ...
    # returns the bourd position stored in that node
    def get_value(self):
        return self.gameState

    # returns the move needed to get to the bourd position stored
    def get_move(self):
        return self.move